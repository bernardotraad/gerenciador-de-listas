        ...
        return (
            <div className="space-y-6">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-foreground">Status da Portaria</h1>
                ...
                <div className="flex flex-col items-center justify-center gap-3 py-20 border border-dashed border-border rounded-2xl">
                    ...
                    <p className="text-muted-foreground text-sm">Nenhum evento ativo hoje</p>
    ...
    return (
        <div className="space-y-6 pb-12">
            ...
            <div className="glass-card border-white/5 rounded-xl p-5">
                <p className="text-xs text-muted-foreground uppercase tracking-widest font-medium mb-1">Evento de hoje</p>
                <p className="text-lg font-bold text-foreground">{eventoHoje.nome}</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {/* Presentes */}
                <div className="glass-card border-white/5 rounded-xl p-5 space-y-3">
                    <div className="flex items-center justify-between">
                        <p className="text-xs text-muted-foreground uppercase tracking-widest font-medium">Presentes</p>
                        <CheckCircle className="w-4 h-4 text-emerald-400" />
                    </div>
                    <p className="text-3xl font-bold text-foreground">{presentes.length}</p>
                    <div>
                        <p className="text-xs text-muted-foreground mb-1">de {eventoHoje.capacidade} vagas ({taxaOcupacao}%)</p>
                ...
                <div className="glass-card border-white/5 rounded-xl p-5 space-y-3">
                    <div className="flex items-center justify-between">
                        <p className="text-xs text-muted-foreground uppercase tracking-widest font-medium">Total Check-ins</p>
                        <Users className="w-4 h-4 text-sky-400" />
                    </div>
                    <p className="text-3xl font-bold text-foreground">{allCheckins.length}</p>
                    <p className="text-xs text-muted-foreground">inclui saídas registradas</p>
                </div>
                ...
                <div className="glass-card border-white/5 rounded-xl p-5 space-y-3">
                    ...
                    <p className="text-3xl font-bold text-foreground">{operadoresIds.length}</p>
                ...
                <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                        {operadoresIds.map((opId) => (
                            <div
                                key={opId}
                                className="glass-card border-white/5 rounded-xl p-4 flex items-center justify-between"
                            >
                                <div className="flex items-center gap-3">
                        ...
                        {Object.entries(tipoCounts).map(([tipo, count]) => {
                            ...
                            return (
                                <div key={tipo} className="space-y-1">
                                    ...
                                    <div className="w-full bg-muted border border-border rounded-full h-1.5">
                                        <div
            ...
            <div className="space-y-3">
                ...
                {presentes.length === 0 ? (
                    <div className="flex flex-col items-center gap-3 py-12 border border-dashed border-border rounded-2xl">
                        <Users className="w-7 h-7 text-muted-foreground" />
                        <p className="text-muted-foreground text-sm">Nenhum convidado presente ainda</p>
                    </div>
                ) : (
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-white/5">
                                    <th className="text-left px-4 py-3 text-xs font-medium text-muted-foreground uppercase tracking-widest">Nome</th>
                                    <th className="text-left px-4 py-3 text-xs font-medium text-muted-foreground uppercase tracking-widest hidden sm:table-cell">Lista</th>
                                    <th className="text-right px-4 py-3 text-xs font-medium text-muted-foreground uppercase tracking-widest">Entrada</th>
                                </tr>
                            </thead>
                                ...
                                {presentes.map((c) => (
                                    <tr key={c.id} className="hover:bg-muted/30 transition-colors">
                                        ...
                                        <td className="px-4 py-3 hidden sm:table-cell">
                                            {c.lista_tipo_nome ? (
                                                <span className="text-xs font-medium px-2 py-0.5 border border-primary/50 text-primary bg-primary/10 rounded-full">
                                                    {c.lista_tipo_nome}
                                                </span>
                                            ) : (
