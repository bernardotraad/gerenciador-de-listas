...
export function SubmissaoCard({
    ...
    return (
        <div
            className="glass-card border-white/5 rounded-xl p-4 space-y-2 cursor-pointer hover:border-white/20 transition-colors select-none"
            onClick={() => setOpen((v) => !v)}
        >
            <div className="flex items-center justify-between gap-2">
                <p className="text-muted-foreground text-xs">{nomes.length} nome{nomes.length !== 1 ? 's' : ''}</p>
            ...
            {open && nomes.length > 0 && (
                <ul className="pt-2 border-t border-white/5 space-y-1 max-h-48 overflow-y-auto">
                    {nomes.map((nome, i) => (
                        <li key={i} className="text-foreground text-xs py-0.5">{nome}</li>
                    ))}
                </ul>
            )}
        </div>
